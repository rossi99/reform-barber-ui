...
async function loadBookings() {
  ...
  try {
    ...
    if (bookingStatusFilter.value !== "all")
      query.status = bookingStatusFilter.value;
    if (bookingBarberFilter.value !== "all")
      query.barber = bookingBarberFilter.value;
    if (bookingDateFilter.value) query.date = bookingDateFilter.value;
    bookings.value = await api<ApiBooking[]>("/api/founder/bookings", {
      query,
    });
  } catch {
...
function svcDirty(s: Svc): boolean {
  return (
    s.name !== s.orig.name ||
    s.durationMins !== s.orig.durationMins ||
    s.pricePence !== s.orig.pricePence
  );
}
...
async function togglePublish(s: Svc) {
  ...
  try {
    await api(`/api/founder/services/${s.id}/publish`, {
      method: "PATCH",
      body: { published: next },
    });
    s.published = next;
...
watch(
  [bookingStatusFilter, bookingBarberFilter, bookingDateFilter],
  loadBookings,
);
</script>
  ...
  <main class="admin-main">
    ...
    <header class="ledger-head">
      <div class="ledger-head__inner">
        ...
        <p class="greeting">
          The whole site, one view -
          <span class="brass-accent">users, bookings, the menu</span>, and your
          own chair time.
        </p>
          ...
          <div class="cell">
            <span class="k"
              ...
              <small class="cell-sub"
                >- {{ stats?.from }} → {{ stats?.to }}</small
              ></span
            ><span class="v"
              ><span class="brass-accent">£</span
              >{{ stats ? (stats.totalPence / 100).toFixed(2) : "-" }}</span
            >
          </div>
          <div class="cell">
            <span class="k">Confirmed</span
            ><span class="v">{{ stats?.confirmedCount ?? "-" }}</span>
          </div>
          <div class="cell">
            <span class="k">Cancelled</span
            ><span class="v">{{ stats?.cancelledCount ?? "-" }}</span>
          </div>
          <div class="cell">
            <span class="k">Users on file</span
            ><span class="v">{{ users.length || "-" }}</span>
          </div>
          <div class="cell">
            <span class="k">Admins</span
            ><span class="v">{{ adminCount || "-" }}</span>
          </div>
          <div class="cell action">
            <a class="btn btn--solid" href="#users"
              >Manage Users <span class="arrow">→</span></a
            >
          </div>
        </div>
        ...
        <div class="kpis">
          <div class="kpi">
            ...
            <span class="v"
              ...
              >{{ stats ? (stats.totalPence / 100).toFixed(2) : "0.00" }}</span
            >
          </div>
          <div class="kpi">
    ...
    <section id="users" class="sec">
      <div class="wrap">
        <div class="sec-head">
          ...
          <div class="aside">
            <span
              ><b>{{ users.length }}</b> on file</span
            ><span
              >{{ adminCount }} admin{{ adminCount === 1 ? "" : "s" }}</span
            >
          </div>
        </div>
        ...
        <div v-else-if="users.length === 0" class="loading-note">
          No users yet.
        </div>
      </div>
    </section>
    ...
    <section id="bookings" class="sec sec--alt">
      <div class="wrap">
        <div class="sec-head">
          ...
          <div class="aside">
            <span
              ><b>{{ bookings.length }}</b> shown</span
            >
          </div>
        </div>
        ...
        <div class="diary-bar">
          <div class="filters" role="tablist">
            ...
            <button
              ...
              @click="bookingStatusFilter = 'completed'"
            >
              Completed
            </button>
            <button
        ...
        <div>
          ...
          >
            ...
            <div class="appt__action">
              <span v-if="b.status === 'confirmed'" class="pill is-upcoming"
      ...
      <div class="wrap">
        <div class="sec-head">
          ...
          <div class="aside">
            <span
              ><b
                >{{ services.filter((s) => s.published).length }} live</b
              ></span
    ...
    <section id="mine" class="sec sec--alt">
      <div class="wrap">
        ...
        <div v-if="myBookingsLoading" class="loading-note">
...
.user-row.is-self {
  background: rgba(var(--brass-rgb), 0.04);
}
.cell-name {
  font-family: var(--serif);
  font-size: 18px;
...
.appt {
  display: grid;
  grid-template-columns: 140px 1.3fr 1fr 1fr auto;
  gap: 32px;
  padding: 24px 0;
  border-bottom: 1px solid var(--rule);
  align-items: center;
...
.appt[data-status="completed"] .appt__date {
...
@media (max-width: 1100px) {
  ...
  .svc-row {
    grid-template-columns: 40px 1fr 90px 90px auto;
    gap: 12px;
  }
}
